...
# Cette fonction genere toutes les cases de notre tableau d'echantillons
def build_samples(filename):
    ...
    samples = [os.path.join(sample_dir, s) for s in samples] # On recupere le chemin complet de chaque image
    samples.sort(key=os.path.getmtime) # On tri par date

    print("Cleaning and building samples...")
    for i in samples: # On traite chaque echantillon
        img = cv2.imread(i) # Ouverture avec OpenCV
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # Passage en nuance de gris pour plus de precision
        tmp_arr.append(gray) # on rempli notre ligne 
        j = j + 1
        if j > 54:
            # on passe a la ligne suivante (chaque ligne represente
            # une lettre et contient autant de cases que d'echantillons de la lettre)
...
def train_knn(filename, cells):
    ...
    gray_test = cv2.cvtColor(img_test, cv2.COLOR_BGR2GRAY) # Passage en nuance de gris pour plus de precision
    ...
    # On applati notre tableau pour que chaque case ne soit representee
    ...
    # On cree un tableau contenant les decimaux ASCII de chacun des caracteres de nos echantillons
    # le tableau est de la meme taille que "train", il contient donc 54 cases pour chaque decimale ASCII
    # knn pourra ainsi lier chaque image a son index ASCII et nous la retourner
    ...
    knn = cv2.KNearest() # On cree notre instance de KNN, l'algo du voisin le plus proche
    knn.train(train,train_labels) # On entraine notre programme avec nos echantillons
...
def handle_results(ret, result, neighbours, dist):
    ...
    # On set le padding selon si le resultat est une minuscule ou une majuscule
    # Le padding nous permettra de verifier si la lettre opposee n'est pas egalement dans les voisins. Exemple :
    # - Si le resultat est un "c" minuscule a 60% et qu'il y a le "C" majuscule dans les voisins proches a 20%
    #   on considere qu'on a trouve "c" avec 80% de precision
    ...
    # Et on affiche les voisins que kNN nous a egalement retourne
